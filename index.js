...
function reset() {
  ...
  let new_size = prompt("Enter new size of grids");
...
function change_color() {
  switch (color_type) {
    ...
    case "rainbow": {
      ...
      let randomColorPos = Math.floor(Math.random() * colorList.length);
      this.style.backgroundColor = colorList[randomColorPos];
      break;
    }
    case "grayscale": {
      let colorList = ["#BEBEBE", "#A0A0A0", "#D8D8D8", "#505050"];
      let randomColorPos = Math.floor(Math.random() * colorList.length);
      this.style.backgroundColor = colorList[randomColorPos];
      break;
    }
